...
/// a PLY point cloud using reservoir sampling.  Optionally uploads to the enterprise endpoint.
///
/// Environment variables:
///   AMENTUM_SCAN_RADIUS_FT      — scan radius in feet          (default: 30)
///   AMENTUM_SCAN_MAX_ELEMENTS   — max elements to collect       (default: 500)
///   AMENTUM_SCAN_MAX_POINTS     — max points in PLY output      (default: 500000)
...
public class ScanAndUploadPointsCommand : IExternalCommand
{
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        ...
        if (centre is null)
        {
            TaskDialog.Show("Amentum — Scan Points",
        ...
        string outPath  = System.IO.Path.Combine(tempDir,
            $"Amentum_{stem}_{DateTime.Now:yyyyMMdd_HHmmss}.ply");

        try
        {
            EnvironmentScanService.ExportPointsPly(
                doc, scannedElements, outPath, maxPoints,
                viewForDetail: doc.ActiveView);
        }
        catch (Exception ex)
        {
            message = $"PLY export failed: {ex.Message}";
